...
// 本文件只执行 gmb.citizenwallet.android.ci 的 build-apk Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenwallet.android.ci","job":"build-apk"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"echo \"CARGO_TARGET_DIR=$RUNNER_TEMP/citizenwallet/cargo-target\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_ANDROID_DIR=$RUNNER_TEMP/citizenwallet/native/android\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_IOS_DIR=$RUNNER_TEMP/citizenwallet/native/ios\" >> \"$GITHUB_ENV\"\n"},"1":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" prepare"},"2":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" wire"},"3":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" sanitize"},"4":{"shell":"bash","source":"node citizenchain/scripts/generate-logo-assets.mjs --check"},"5":{"shell":"bash","source":"test \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\n"},"6":{"shell":"bash","source":"# 版本只读受控工具登记，不读取产品依赖合同中的副本。\nprintf 'version=3.47.2\n' >> \"$GITHUB_OUTPUT\"\n"},"7":{"shell":"bash","source":"# 安装后先验真，再统一准备目标平台缓存与受控修订。\nflutter --version --machine >/dev/null\nplatform=\"android\"\nflutter --version >/dev/null\n"},"8":{"shell":"bash","source":"sdkmanager \"platforms;android-36\" \"build-tools;36.0.0\" \"platform-tools\""},"9":{"shell":"bash","source":"./citizenwallet/scripts/sync-pallet-registry.sh"},"10":{"shell":"bash","source":"flutter pub get --enforce-lockfile\n"},"11":{"shell":"bash","source":"test ! -e analysis_options.yaml && test ! -L analysis_options.yaml\ntrap 'rm -f -- analysis_options.yaml' EXIT\ncp scripts/analysis_options.yaml analysis_options.yaml\nflutter analyze --no-pub lib/qr/scanner test/qr/scanner\nrm -f -- analysis_options.yaml\ntrap - EXIT\nflutter test --no-pub test/qr/scanner\n"},"12":{"shell":"bash","source":"./scripts/build-signer-native.sh android"},"13":{"shell":"bash","source":"test ! -e analysis_options.yaml && test ! -L analysis_options.yaml\ntrap 'rm -f -- analysis_options.yaml' EXIT\ncp scripts/analysis_options.yaml analysis_options.yaml\nflutter analyze --no-fatal-infos\n"},"14":{"shell":"bash","source":"./scripts/build-signer-native.sh host"},"15":{"shell":"bash","source":"flutter test"},"16":{"shell":"bash","source":"node --test citizenwallet/test/release_manifest.test.mjs\n"},"17":{"shell":"bash","source":"flutter build apk --release --target-platform android-arm64"},"18":{"shell":"bash","source":"./gradlew :app:testReleaseUnitTest"},"19":{"shell":"bash","source":"./scripts/build-signer-native.sh verify-android-package build/app/outputs/flutter-apk/app-release.apk"},"20":{"shell":"bash","source":"set -euo pipefail\nGMB_CI_STORE_PASSWORD=\"$(openssl rand -hex 32)\"\nGMB_CI_KEY_PASSWORD=\"$GMB_CI_STORE_PASSWORD\"\nGMB_CI_APKSIGNER=\"$ANDROID_HOME/build-tools/36.0.0/apksigner\"\nexport GMB_CI_STORE_PASSWORD GMB_CI_KEY_PASSWORD\ntest -x \"$GMB_CI_APKSIGNER\"\ntrap 'rm -f \"$RUNNER_TEMP/citizenwallet-ci.p12\"' EXIT\nkeytool -genkeypair -storetype PKCS12 \\n  -keystore \"$RUNNER_TEMP/citizenwallet-ci.p12\" \\n  -storepass:env GMB_CI_STORE_PASSWORD -keypass:env GMB_CI_KEY_PASSWORD -alias ci \\n  -keyalg RSA -keysize 4096 -validity 2 \\n  -dname 'CN=CitizenWallet CI,O=GMB,C=US'\n\"$GMB_CI_APKSIGNER\" sign --ks \"$RUNNER_TEMP/citizenwallet-ci.p12\" \\n  --ks-type PKCS12 --ks-key-alias ci \\n  --ks-pass env:GMB_CI_STORE_PASSWORD --key-pass env:GMB_CI_KEY_PASSWORD \\n  --out citizenwallet/build/app/outputs/flutter-apk/公民钱包-CI.apk \\n  citizenwallet/build/app/outputs/flutter-apk/app-release.apk\n\"$GMB_CI_APKSIGNER\" verify --verbose --print-certs citizenwallet/build/app/outputs/flutter-apk/公民钱包-CI.apk\nunset GMB_CI_STORE_PASSWORD GMB_CI_KEY_PASSWORD\n"},"21":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" record\n"},"22":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" prune"},"23":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" record\n"},"24":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/android/build-apk/execute.mjs\" prune"}});
