...
// 本文件只执行 gmb.citizenwallet.ios.ci 的 ios Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenwallet.ios.ci","job":"ios"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"echo \"CARGO_TARGET_DIR=$RUNNER_TEMP/citizenwallet/cargo-target\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_ANDROID_DIR=$RUNNER_TEMP/citizenwallet/native/android\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_IOS_DIR=$RUNNER_TEMP/citizenwallet/native/ios\" >> \"$GITHUB_ENV\"\n"},"1":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" prepare"},"2":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" wire"},"3":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" sanitize"},"4":{"shell":"bash","source":"test \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\n"},"5":{"shell":"bash","source":"# 版本只读受控工具登记，不读取产品依赖合同中的副本。\nprintf 'version=3.47.2\n' >> \"$GITHUB_OUTPUT\"\n"},"6":{"shell":"bash","source":"# 安装后先验真，再统一准备目标平台缓存与受控修订。\nflutter --version --machine >/dev/null\nplatform=\"ios\"\nflutter --version >/dev/null\n"},"7":{"shell":"bash","source":"./citizenwallet/scripts/sync-pallet-registry.sh"},"8":{"shell":"bash","source":"flutter pub get --enforce-lockfile\n"},"9":{"shell":"bash","source":"./scripts/build-signer-native.sh ios"},"10":{"shell":"bash","source":"flutter build ios --release --no-codesign"},"11":{"shell":"bash","source":"./scripts/build-signer-native.sh verify-ios-package build/ios/iphoneos/Runner.app"},"12":{"shell":"bash","source":"codesign --force --deep --sign - citizenwallet/build/ios/iphoneos/Runner.app\ncodesign --verify --deep --strict citizenwallet/build/ios/iphoneos/Runner.app\n"},"13":{"shell":"bash","source":"set -euo pipefail\nmkdir -p citizenwallet/build/release-candidate/ios\nasset='公民钱包-iOS-CI.app.zip'\nditto -c -k --keepParent citizenwallet/build/ios/iphoneos/Runner.app \\n  \"citizenwallet/build/release-candidate/ios/${asset}\"\n"},"14":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" record\n"},"15":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" prune"},"16":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" record\n"},"17":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/ios/execute.mjs\" prune"}});
