...
// 本文件只执行 gmb.citizenwallet.ios.ci 的 check Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenwallet.ios.ci","job":"check"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"echo \"CARGO_TARGET_DIR=$RUNNER_TEMP/citizenwallet/cargo-target\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_ANDROID_DIR=$RUNNER_TEMP/citizenwallet/native/android\" >> \"$GITHUB_ENV\"\necho \"CITIZENWALLET_NATIVE_IOS_DIR=$RUNNER_TEMP/citizenwallet/native/ios\" >> \"$GITHUB_ENV\"\n"},"1":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" prepare"},"2":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" wire"},"3":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" sanitize"},"4":{"shell":"bash","source":"node citizenchain/scripts/generate-logo-assets.mjs --check"},"5":{"shell":"bash","source":"test \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\n"},"6":{"shell":"bash","source":"# 版本只读受控工具登记，不读取产品依赖合同中的副本。\nprintf 'version=3.47.2\n' >> \"$GITHUB_OUTPUT\"\n"},"7":{"shell":"bash","source":"# 安装后先验真，再统一准备目标平台缓存与受控修订。\nflutter --version --machine >/dev/null\nplatform=\"ios\"\nflutter --version >/dev/null\n"},"8":{"shell":"bash","source":"./citizenwallet/scripts/sync-pallet-registry.sh"},"9":{"shell":"bash","source":"flutter pub get --enforce-lockfile\n"},"10":{"shell":"bash","source":"test ! -e analysis_options.yaml && test ! -L analysis_options.yaml\ntrap 'rm -f -- analysis_options.yaml' EXIT\ncp scripts/analysis_options.yaml analysis_options.yaml\nflutter analyze --no-pub lib/qr/scanner test/qr/scanner\nrm -f -- analysis_options.yaml\ntrap - EXIT\nflutter test --no-pub test/qr/scanner\n"},"11":{"shell":"bash","source":"test ! -e analysis_options.yaml && test ! -L analysis_options.yaml\ntrap 'rm -f -- analysis_options.yaml' EXIT\ncp scripts/analysis_options.yaml analysis_options.yaml\nflutter analyze --no-fatal-infos\n"},"12":{"shell":"bash","source":"./scripts/build-signer-native.sh host"},"13":{"shell":"bash","source":"flutter test"},"14":{"shell":"bash","source":"node --test citizenwallet/test/release_manifest.test.mjs\n"},"15":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" record\n"},"16":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" prune"},"17":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" record\n"},"18":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenwallet/scripts/ci/ios/check/execute.mjs\" prune"}});
