...
const cache = remote ? process.env.XDG_CACHE_HOME : process.env.TATA_CONSOLE_BUILD_WORK_DIR;
...
if (!ownerRoot || !isAbsolute(ownerRoot) || !cache || !isAbsolute(cache)
  || resolve(cache) !== cache || (remote
    ...
    : !work || resolve(work) !== work
      || !work.startsWith(resolve(ownerRoot, 'tataconsole/work/gmb/citizenserve') + sep)
