...
// 本文件只执行 gmb.citizenserve.cloudflare.ci 的 check Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenserve.cloudflare.ci","job":"check"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" prepare"},"1":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" wire"},"2":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" sanitize"},"3":{"shell":"bash","source":"test \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\n"},"4":{"shell":"bash","source":"npm ci --no-audit --no-fund\n"},"5":{"shell":"bash","source":"npm run types:check"},"6":{"shell":"bash","source":"npm run typecheck"},"7":{"shell":"bash","source":"npm test"},"8":{"shell":"bash","source":"npm exec -- wrangler d1 execute DB --config scripts/wrangler.toml --local \\n  --persist-to \"$RUNNER_TEMP/citizenserve-cloudflare-d1\" \\n  --file schema/citizenserve.sql\nnpm exec -- wrangler d1 execute CITIZENCHAIN_DOWNLOAD_DB --config scripts/wrangler.toml --local \\n  --persist-to \"$RUNNER_TEMP/citizenserve-cloudflare-d1\" \\n  --file schema/download.sql\ntest ! -e migrations\n"},"9":{"shell":"bash","source":"npm exec -- wrangler deploy --config scripts/wrangler.toml --dry-run --outdir \"$RUNNER_TEMP/citizenserve-cloudflare-bundle\""},"10":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/index.mjs\" action --project citizenserve --bundle \"$RUNNER_TEMP/citizenserve-cloudflare-bundle/index.js\" --output \"$RUNNER_TEMP/citizenserve-cloudflare-candidate\" --git-sha \"$GMB_SOURCE_SHA\" --archive \"$RUNNER_TEMP/citizenserve-cloudflare-release.tgz\""},"11":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" record\n"},"12":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" prune"},"13":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" sanitize\nnode \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" record\n"},"14":{"shell":"bash","source":"node \"$GITHUB_WORKSPACE/citizenserve/scripts/ci/cloudflare/check/execute.mjs\" prune"}});
