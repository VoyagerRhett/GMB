...
// 本文件只执行 gmb.citizenchain-node.linux-arm.ci 的 changes Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenchain-node.linux-arm.ci","job":"changes"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"node citizenchain/scripts/generate-logo-assets.mjs --check"},"1":{"shell":"bash","source":"set -euo pipefail\ntest \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\necho \"citizenchain=true\" >> \"$GITHUB_OUTPUT\"\necho \"desktop=true\" >> \"$GITHUB_OUTPUT\"\ntest \"$GMB_NODE_PLATFORM\" = linux-arm\necho 'desktop_matrix=[{\"platform\":\"linux-arm\",\"name\":\"LinuxARM\",\"os\":\"ubuntu-24.04-arm\",\"artifact\":\"公民链LinuxARM\",\"installer_name\":\"citizenchain-node-LinuxARM.deb\",\"ci_bundle_targets\":\"deb\",\"updater_asset_name\":\"citizenchain-node-LinuxARM.AppImage\",\"updater_manifest_name\":\"citizenchain-node-latest-LinuxARM.json\",\"expected_runner_arch\":\"ARM64\",\"deb_arch\":\"arm64\",\"cache_key\":\"citizenchain-linux-arm\"}]' >> \"$GITHUB_OUTPUT\"\n"}});
