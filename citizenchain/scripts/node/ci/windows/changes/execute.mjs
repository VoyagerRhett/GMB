...
// 本文件只执行 gmb.citizenchain-node.windows.ci 的 changes Job；阶段编号由本仓唯一 Workflow 固定，禁止接收其它身份。
export const EXACT_REMOTE_JOB_IDENTITY = Object.freeze({"pipeline":"gmb.citizenchain-node.windows.ci","job":"changes"});
...
const workflowSteps = Object.freeze({"0":{"shell":"bash","source":"node citizenchain/scripts/generate-logo-assets.mjs --check"},"1":{"shell":"bash","source":"set -euo pipefail\ntest \"$(git rev-parse HEAD)\" = \"$GMB_SOURCE_SHA\"\necho \"citizenchain=true\" >> \"$GITHUB_OUTPUT\"\necho \"desktop=true\" >> \"$GITHUB_OUTPUT\"\ntest \"$GMB_NODE_PLATFORM\" = windows\necho 'desktop_matrix=[{\"platform\":\"windows\",\"name\":\"Windows\",\"os\":\"windows-2025\",\"artifact\":\"公民链Windows\",\"installer_name\":\"citizenchain-node-Windows.exe\",\"ci_bundle_targets\":\"nsis\",\"updater_asset_name\":\"\",\"updater_manifest_name\":\"citizenchain-node-latest-Windows.json\",\"expected_runner_arch\":\"X64\",\"cache_key\":\"citizenchain-windows\"}]' >> \"$GITHUB_OUTPUT\"\n"}});
