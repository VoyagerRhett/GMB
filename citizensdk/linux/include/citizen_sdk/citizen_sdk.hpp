...
class Host final {
 public:
  explicit Host(const Config &config) {
    const std::string storage = config.storage_root.string();
    const std::string assets = config.asset_root.string();
    ...
    native.gtk_parent_window = config.gtk_parent_window;
  ...
  void close() {
    ...
    throw_if_error(citizensdk_host_destroy(host_),
                   "CitizenSDK Host close failed");
    host_ = 0;
    sdk_ = 0;
  }
