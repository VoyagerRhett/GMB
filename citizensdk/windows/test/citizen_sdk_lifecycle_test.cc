...
int main() {
  ...
  // 显式 destroy 遇到另一个 API lease 立即 BUSY，绝不在 callback/Win32
  ...
  const auto retire_window = host_bridge.find("parent_window_.retire()", destroy_core);
  assert(retire_window != std::string::npos && destroy_core < retire_window);
  ...
  assert(foreign_boundary != std::string::npos &&
         ...
         clear_core_callback < destroy_core && retire_window < retire_services &&
