...
Value wallet_state(citizensdk_result_handle_t result) {
  ...
  if (profile_info.present != 0) {
    ...
    profile_value = tuple({Value::integer(profile_info.wallet_index),
        ...
        Value::string(std::to_string(profile_info.created_at_millis)),
        hex(profile_info.master_account_id.bytes), hex(profile_info.active_account_id.bytes),
        Value::list(std::move(hot_accounts))});
  }
