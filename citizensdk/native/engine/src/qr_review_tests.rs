...
use citizen_sdk_contracts::{ChainIdentity, ContractErrorCode, FinalizedBlockRef, Hash32, RuntimeContext, RuntimeVersion, Sr25519PublicKey};
use citizen_sdk_qr::SignRequest;
use subxt_core::ext::codec::{Compact, Encode};

use crate::{error::EngineError, qr_review::{decode_review, QrReview}};
use std::sync::{Arc, Mutex, atomic::{AtomicUsize, Ordering}};
use citizen_sdk_contracts::{ContractError, ContractFuture, ContractStream, ExportedChainState, ExtrinsicWatchEvent, SignedExtrinsic, StateImportReceipt, SubmittedExtrinsic, VerifiedBlockRef, VerifiedChainClient};

const METADATA: &str = include_str!("../../../test/transaction/citizenchain-runtime-v14-metadata.hex");
...
fn fixture() -> (SignRequest, RuntimeContext, usize) {
    ...
    let request = SignRequest {
        request_id: "0123456789abcdef".to_owned(), expires_at: i64::MAX as u64,
        action: 0x0400,
        signer_public_key: Sr25519PublicKey::from_bytes(bytes(vector["transfer"]["source_account_id"].as_str().unwrap()).try_into().unwrap()),
    ...
    let context = RuntimeContext::try_new(finalized.verified(), RuntimeVersion::new(0, 0), bytes(METADATA)).unwrap();
...
fn review(request: SignRequest, context: &RuntimeContext) -> Result<QrReview, EngineError> {
    decode_review(&ChainIdentity::citizenchain(), FinalizedBlockRef::from_parts(context.block().hash(), context.block().number()), context, request)
}
...
fn complete_scale_payload_exposes_all_confirmed_facts_without_wallet() {
    ...
    assert_eq!(reviewed.genesis_hash(), ChainIdentity::citizenchain().genesis_hash());
...
fn wrong_chain_runtime_and_checkpoint_never_become_a_review() {
    ...
    let newer = RuntimeContext::try_new(context.block(), RuntimeVersion::new(1, 0), context.metadata().to_vec()).unwrap();
...
fn noncanonical_nonce_and_unknown_metadata_hash_mode_are_rejected() {
    let (original, context, call_len) = fixture();
    let mut request = original.clone();
    request.review_payload.splice(call_len + 1..call_len + 2, [0x1d, 0x00]);
    code(review(request, &context), ContractErrorCode::Decode);
    let mut request = original;
...
fn confirmed_review_allows_head_progress_but_not_request_changes() {
    ...
    let next = RuntimeContext::try_new(FinalizedBlockRef::from_parts(Hash32::from_bytes([0x44; 32]), 78).verified(), context.version(), context.metadata().to_vec()).unwrap();
...
fn scale_string_review_makes_bidi_and_invisible_controls_visible_without_changing_chinese() {
    ...
    assert!(!rendered.chars().any(|c| matches!(c, '\u{202e}' | '\u{2066}' | '\u{200b}' | '\u{feff}')));
    assert!(value.as_str().unwrap().contains('\u{202e}'), "展示转义不得改原值");
}
...
fn unused<T>() -> ContractFuture<'static, T> {
    Box::pin(async { Err(ContractError::new(ContractErrorCode::Unsupported, "QR 审阅不应读取此接口")) })
}

impl VerifiedChainClient for ReviewClient {
    fn identity(&self) -> ContractFuture<'_, ChainIdentity> { Box::pin(async { Ok(ChainIdentity::citizenchain()) }) }
    fn get_best_head(&self) -> ContractFuture<'_, VerifiedBlockRef> { unused() }
    fn get_finalized_head(&self) -> ContractFuture<'_, FinalizedBlockRef> { Box::pin(async { Ok(self.finalized) }) }
    fn get_runtime_context_at(&self, _block: VerifiedBlockRef) -> ContractFuture<'_, RuntimeContext> {
        let gate = self.runtime_gate.lock().unwrap().take();
        self.reads.fetch_add(1, Ordering::SeqCst);
        Box::pin(async move { if let Some(gate) = gate { let _ = gate.await; } Ok(self.context.clone()) })
    }
    fn get_storage_at(&self, _: VerifiedBlockRef, _: Vec<u8>) -> ContractFuture<'_, Option<Vec<u8>>> { unused() }
    fn get_storage_batch_at(&self, _: VerifiedBlockRef, _: Vec<Vec<u8>>) -> ContractFuture<'_, Vec<Option<Vec<u8>>>> { unused() }
    fn get_block_extrinsics_at(&self, _: VerifiedBlockRef) -> ContractFuture<'_, Vec<Vec<u8>>> { unused() }
    fn submit_extrinsic(&self, _: SignedExtrinsic) -> ContractFuture<'_, SubmittedExtrinsic> { unused() }
    fn watch_extrinsic(&self, _: SignedExtrinsic) -> ContractStream<'_, ExtrinsicWatchEvent> { Box::pin(futures::stream::empty()) }
    fn export_state(&self) -> ContractFuture<'_, ExportedChainState> { unused() }
    fn import_state(&self, _: ExportedChainState) -> ContractFuture<'_, StateImportReceipt> { unused() }
}

fn client(context: RuntimeContext) -> ReviewClient {
    ReviewClient { finalized: FinalizedBlockRef::from_parts(context.block().hash(), context.block().number()), context, reads: AtomicUsize::new(0), runtime_gate: Mutex::new(None) }
}
...
fn mortal_checkpoint_is_verified_against_exact_finalized_chain() {
    futures::executor::block_on(async {
        ...
        request.review_payload.splice(call_len..call_len + 1, encoded_era.clone());
        let checkpoint = call_len + encoded_era.len() - 1 + 44;
        request.review_payload[checkpoint..checkpoint + 32].copy_from_slice(context.block().hash().as_bytes());
        let provider = client(context);
        let reviewed = crate::qr_review::review_request(&provider, request.clone()).await.unwrap();
        assert_eq!(reviewed.era(), "mortal(period=64,phase=13,birth=77)");
        request.review_payload[checkpoint] ^= 1;
        code(crate::qr_review::review_request(&provider, request).await, ContractErrorCode::Integrity);
    });
}
...
fn runtime_from_another_block_is_rejected_before_scale_review() {
    ...
    code(futures::executor::block_on(crate::qr_review::review_request(&provider, request)), ContractErrorCode::Integrity);
}
...
fn qr_chain_review_requires_running_and_holds_lifecycle_until_real_work_drains() {
    ...
    futures::executor::block_on(async {
        ...
        let components = crate::EngineComponents::new(Some(provider.clone()), None, None, None, None, None, None, None)
            .with_modules(Modules::try_new(Modules::QR | Modules::CHAIN).unwrap());
        let engine = crate::CitizenEngine::new(components);
        engine.update_capabilities(CapabilityName::ALL.into_iter().map(crate::CapabilityProbe::ready).collect()).unwrap();
        assert!(engine.review_qr_sign_request(request.clone()).await.is_err());
        assert_eq!(provider.reads.load(Ordering::SeqCst), 0, "不能暗启或读取未运行链");
